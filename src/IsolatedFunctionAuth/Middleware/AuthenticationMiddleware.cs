...
namespace IsolatedFunctionAuth.Middleware
{
    public class AuthenticationMiddleware : IFunctionsWorkerMiddleware
    {
        ...
        {
            string aadInstance = configuration.GetSection("AzureActiveDirectory").GetValue<string>("Instance");
            string aadTenant = configuration.GetSection("AzureActiveDirectory").GetValue<string>("TenantId");
            var authority = String.Format(CultureInfo.CurrentCulture, "{0}/{1}/v2.0", aadInstance, aadTenant);
            var audience = configuration.GetSection("AzureActiveDirectory").GetValue<string>("ClientId");
