...
namespace IsolatedFunctionAuth.Middleware
{
    public class AuthorizationMiddleware : IFunctionsWorkerMiddleware
    {
        ...
        private static bool AuthorizeDelegatedPermissions(FunctionContext context, ClaimsPrincipal principal)
        {
            ...
            if (acceptedUserRoles.Count > 0) // The API has required User Roles
            {
                var userRoles = principal.FindAll(ClaimTypes.Role);
                userHasAcceptedRole = userRoles.Any(ur => acceptedUserRoles.Contains(ur.Value)); // check User role
            }

            bool callerHasAcceptedScope = true;
            if (acceptedScopes.Count > 0) // The API has required Application scope
            {
